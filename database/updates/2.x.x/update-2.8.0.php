...
try {
	...
	// home_sections
	DB::table('home_sections')->where('method', 'getSearchForm')->update(['name' => 'Search Form Area']);
	DB::table('home_sections')->where('method', 'getBottomAdvertising')->update(['name' => 'Advertising #2']);
	DB::table('home_sections')->where('method', 'getCategories')->update(['options' => '{"max_items":null,"cache_expiration":null}']);
	
	$homeSection = \App\Models\HomeSection::where('method', 'getTopAdvertising')->first();
