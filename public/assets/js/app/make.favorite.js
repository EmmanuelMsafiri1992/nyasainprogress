...
function savePost(el) {
	...
	let ajax = $.ajax({
		...
		beforeSend: function() {
			...
			if (iconEl) {
				iconEl.removeClass('fa-regular fa-bookmark');
	...
	ajax.done(function (xhr) {
		...
		if (typeof xhr.isLogged === 'undefined') {
			...
			if (iconEl) {
				...
				iconEl.addClass('fa-regular fa-bookmark').removeAttr('role aria-hidden');
			}
		...
		if (xhr.isSaved === true) {
			if ($(el).hasClass('btn')) {
				...
				$(el).html('<span class="fa-solid fa-bookmark"></span> ' + lang.labelSavePostRemove);
			}
			...
			if ($(el).hasClass('btn')) {
				...
				$(el).html('<span class="fa-regular fa-bookmark"></span> ' + lang.labelSavePostSave);
			}
