...
class HomeSectionController extends BaseController
{
	...
	public function show($method): \Illuminate\Http\JsonResponse
	{
		...
		// Get all homepage sections
		$cacheId = $countryCode . '.homeSections.' . $method . '.includingNonActive.' . $isUnactivatedIncluded;
		$section = cache()->remember($cacheId, $this->cacheExpiration, function () use ($countryCode, $method, $isUnactivatedIncluded) {
			$section = null;
			...
			if (config('plugins.domainmapping.installed')) {
				try {
					...
					$section = $section->where('country_code', $countryCode)->where('method', $method)->first();
				} catch (\Throwable $e) {
				}
			}
			...
			if (empty($section)) {
				...
				$section = $section->where('method', $method)->first();
			}
